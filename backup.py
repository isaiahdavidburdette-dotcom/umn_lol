...
def get_all_current_courses(campus="UMNTC"):
    """Get all courses offered this semester"""
    term = get_current_term()
...
def calculate_gpa_for_course(course_name):
    """Calculate average GPA for a course across all sections"""
...
async def search(ctx, *, keyword: str):
    ...
    matches = df[
        (df['FULL_NAME'].str.contains(keyword, na=False)) |
        (df['DESCR'].str.contains(keyword, na=False))
        ]['FULL_NAME'].unique()
    ...
    if len(matches) > 20:
        embed.set_footer(text=f"Showing 20 of {len(matches)} results")
...
async def easy(ctx, limit: int = 10):
    ...
    for course in df['FULL_NAME'].unique():
        gpa, grade_dist = calculate_gpa_for_course(course)
        if gpa > 0:
            course_gpas[course] = gpa

    top_courses = sorted(course_gpas.items(), key=lambda x: x[1], reverse=True)[:limit]

    result = [f"**{course}**: {gpa:.2f}" for course, gpa in top_courses]
...
async def hard(ctx, limit: int = 10):
    ...
    bottom_courses = sorted(course_gpas.items(), key=lambda x: x[1])[:limit]

    result = [f"**{course}**: {gpa:.2f}" for course, gpa in bottom_courses]
...
@bot.command()
async def compare(ctx, course1: str, course2: str):
    """
    Compare two courses
    ...
    if gpa1 == 0 or gpa2 == 0:
        await ctx.send("❌ One or both courses not found")
        return

    sections1 = df[df['FULL_NAME'] == course1]['CLASS_SECTION'].nunique()
    sections2 = df[df['FULL_NAME'] == course2]['CLASS_SECTION'].nunique()

    embed = discord.Embed(title="⚖️ Course Comparison", color=discord.Color.orange())
    embed.add_field(name=f"{course1}", value=f"GPA: {gpa1:.2f}\nSections: {sections1}", inline=True)
    embed.add_field(name=f"{course2}", value=f"GPA: {gpa2:.2f}\nSections: {sections2}", inline=True)
...
async def pick(ctx, dept: str, difficulty: str = "easy"):
    ...
    # Get all courses in department from historical data
    dept_courses = df[df['SUBJECT'] == dept]['FULL_NAME'].unique()
    ...
    # Calculate GPAs for all department courses
    course_gpas = {}
    for course in dept_courses:
        gpa, _ = calculate_gpa_for_course(course)
        if gpa > 0:
            course_gpas[course] = gpa

    # Sort by difficulty
    if difficulty == "easy":
        sorted_courses = sorted(course_gpas.items(), key=lambda x: x[1], reverse=True)
    else:
        sorted_courses = sorted(course_gpas.items(), key=lambda x: x[1])

    # Check which ones are offered this semester
    available_courses = []
    for course, gpa in sorted_courses[:30]:  # Check top 30
        parts = course.split()
        if len(parts) >= 2:
...
async def openandeasy(ctx, limit: int = 10):
    ...
    # Get courses sorted by GPA
    course_gpas = {}
    for course in df['FULL_NAME'].unique():
        gpa, _ = calculate_gpa_for_course(course)
        if gpa >= 3.0:  # Only consider courses with GPA 3.0+
            course_gpas[course] = gpa
    ...
    for course, gpa in sorted_courses:
