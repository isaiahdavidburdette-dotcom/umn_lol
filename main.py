...
async def openandeasy(ctx, limit: int = 10):
    ...
    await ctx.send(embed=embed)
## help command
# First, remove the default help if you haven't
...
async def help(ctx):
    """
    Custom Help command with fixed UMN Maroon color
    """
    # 0x7A0019 is the official UMN Maroon hex code
    embed = discord.Embed(
        title="🎓 UMN Course Bot Help",
        description="Your guide to navigating classes and grades at UMN.",
        color=0x7A0019
    )
    ...
    embed.add_field(
        ...
        value="`!grade [Course]` - Historical distribution\n"
              "`!compare [C1], [C2]` - Side-by-side GPA\n"
              "`!stats [Course]` - Deep dive stats",
        inline=False
    )

    # Current Registration Section
    embed.add_field(
        name="📅 Registration Tools",
        value="`!sections [Course]` - Live seat counts\n"
              "`!prereq [Course]` - Requirement check\n"
              "`!notify [Course]` - DM when a seat opens",
        inline=False
    )
    ...
    embed.add_field(
        name="💡 Discovery",
        value="`!search [Keyword]` - Find courses\n"
              "`!subject [DEPT]` - List dept courses\n"
              "`!optimize [C1, C2...]` - Find easiest sections",
        inline=False
    )

    embed.set_footer(text="Ski-U-Mah! | Use ! followed by the command.")
